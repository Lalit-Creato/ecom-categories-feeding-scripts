/**
 * Standalone NestJS Seeding Script
 * 
 * Run this file directly to execute all seeding operations:
 * 
 * Using ts-node:
...
export class EcomSeedingService {
  ...
  async fillCategories(jsonFilePath?: string): Promise<{ success: boolean; count: number; message: string }> {
    try {
      ...
      const encodings: BufferEncoding[] = ['utf8', 'utf8', 'latin1'];
      let data: any = null;
...
async function bootstrap() {
  ...
  try {
    logger.log('🚀 Starting E-commerce Seeding Script...');
    logger.log('==========================================');
    ...
    const seedingService = app.get(EcomSeedingService);
    ...
    logger.log('==========================================');
    await seedingService.fillCategories(jsonFilePath);

    // Step 2: Generate Variant Options
    logger.log('\n🤖 STEP 2: Generating Variant Options');
    logger.log('==========================================');
    await seedingService.generateVariantOptions(limit);

    // Step 3: Link Categories to Options
    logger.log('\n🔗 STEP 3: Linking Categories to Options');
    logger.log('==========================================');
    await seedingService.linkCategoryOptions(limit);

    // Cleanup - app.close() will automatically call onModuleDestroy
    await app.close();
